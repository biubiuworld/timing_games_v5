...
def read_csv(parameter):
    input_file = csv.DictReader(open("single_timing_game/configs/demo.csv"))
    parameter_list = []
...
class C(BaseConstants):
    NAME_IN_URL = 'single_timing_game'
    PLAYERS_PER_GROUP = 2
    XMAX = read_csv('XMAX')
    XMIN = read_csv('XMIN')
    YMAX = read_csv('YMAX')
...
class WaitToStart(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        ...
        group.start_timestamp = str(round(time.time(), 1)) 

        group.num_messages = 0
        group.messages_roundzero = 0
        group.num_players = C.NUM_OF_BOTS + C.PLAYERS_PER_GROUP
        
        # assign initial strategies
        strategies = generate_initial_strategies(group, C.NUM_OF_BOTS + C.PLAYERS_PER_GROUP) # generate initial strategies
        for p in group.get_players():
            p.player_id_index = p.id_in_group + 1 # assign human subject player index
            p.player_strategy = strategies[p.player_id_index] #### need modify in the future
        group.str_strategies = transfer_array_to_str(strategies) # store strategies in string format

        bubble_coordinate_array, multiplier_bubble_coordinate_array = generate_bubble_coordinate(group, strategies)
        landscape_coordinate_array, multiplier_landscape_coordinate_array, group.peak_payoff_location = generate_landscape_coordinate(group, strategies)
        ...
        for player_index in range(C.NUM_OF_BOTS + C.PLAYERS_PER_GROUP):
        ...
        for p in group.get_players():
            p.participant.avg_group_payoff_history = avg_group_payoff_history
            p.participant.highcharts_landscape_series = highcharts_landscape_series
            p.participant.highcharts_series = highcharts_series
            p.participant.highcharts_payoff_series = highcharts_payoff_series
            p.remaining_freeze_period = 0
            p.if_freeze_next = 0
            p.if_freeze_now = 0
            p.move=0
            p.participant.if_freeze_next_all = [0]*(C.NUM_OF_BOTS + C.PLAYERS_PER_GROUP)
            # p.participant.remaining_freeze_period_for_all = remaining_freeze_period_for_all
            # p.participant.if_freeze_next = if_freeze_next
            # p.participant.if_freeze_now = if_freeze_now

            Adjustment.create(
                    player=p,
                    group=group,
                    player_id_index=p.player_id_index, # add dat to export table
                    strategy=p.player_strategy,
                    strategy_payoff=strategies_payoffs[p.player_id_index],
                    multiplier_strategy_payoff=multiplier_strategies_payoffs[p.player_id_index],
                    seconds=0,
                    move=0,
                    remaining_freeze=0,
                    if_freeze_next=0,
...
class MyPage(Page):
    ...
    def js_vars(player: Player): #Passing data from Python to JavaScript
        num_players = C.NUM_OF_BOTS + C.PLAYERS_PER_GROUP
    ...
    def live_method(player: Player, data):
        ...
        human_id_index = []
        for p in group.get_players():
            human_id_index.append(p.player_id_index)
        # bots id index
        bots_id_index = list(set(range(num_players)) - set(human_id_index))
        # num of bots move
        num_move = round(C.NUM_OF_BOTS * C.MOVE_PERCENT)
        ...
        if data == {}: #at beginning, when receive none msg, reset the timestamp
            ...
            if group.num_messages % C.PLAYERS_PER_GROUP == 0:   
                group.num_messages = 0

                # convert str strategies to array
                previous_strategies = [round(float(x),2) for x in group.str_strategies.split(',')]
                previous_strategies = np.array(previous_strategies).copy()
                current_strategies = previous_strategies
                for p in group.get_players():
                    current_strategies[p.player_id_index] = p.player_strategy # update real human strategy
                
                selected_bots = random.sample(bots_id_index, num_move)
                for bot_idx in  selected_bots:
                    current_strategies[bot_idx] = group.peak_payoff_location + random.random() * 0.2 - 0.2/2
                
                # move_for_all = [0]*num_players
                # remaining_freeze_period_for_all = [0]*num_players
                for p in group.get_players():
                    if p.player_previous_strategy != p.player_strategy:
                        p.move = 1
                        p.remaining_freeze_period = int(C.FREEZE_PERIOD[p.round_number-1]) 
                    elif p.player_previous_strategy == p.player_strategy:
                        if p.remaining_freeze_period > 0:
                            p.remaining_freeze_period -= 1

                    if p.remaining_freeze_period != 0:
                        p.if_freeze_next = 1
                    else:
                        p.if_freeze_next = 0
                group.str_strategies = transfer_array_to_str(current_strategies)
                ...
                landscape_coordinate_array, multiplier_landscape_coordinate_array, group.peak_payoff_location = generate_landscape_coordinate(player, current_strategies)
                ...
                now_seconds = round(time.time()- float(group.start_timestamp), 1)
                # print(now_seconds)
                ...
                for p in group.get_players():
                    p.participant.avg_group_payoff_history.append([now_seconds,multiplier_avg_strategies_payoffs])#group avg over time (multiplied)
                    ...
                    Adjustment.create(
                        ...
                        seconds=now_seconds,
                        move=p.move,
                        remaining_freeze=p.remaining_freeze_period,
                        if_freeze_next=p.if_freeze_next,
                        if_freeze_now=p.if_freeze_now,
                    )
                    p.if_freeze_now = p.if_freeze_next
                
                if_freeze_next_all = [0]*num_players
                for p in group.get_players():
                    if_freeze_next_all[p.player_id_index] = p.if_freeze_next
                for p in group.get_players():
                    p.participant.if_freeze_next_all = if_freeze_next_all
                ...
                for p in group.get_players():
                    ...
                    for player_index in range(num_players):
                        ...
                        p.participant.highcharts_series[player_index].append(history)
                        payoff_history = [now_seconds, multiplier_strategies_payoffs[player_index]]
                        p.participant.highcharts_payoff_series[player_index].append(payoff_history)
...
def custom_export(players):
    ...
    yield ['session','subperiod', 'period_length', 'xmax','xmin','ymax','ymin','lambda','gamma','rho','freeze_period', 'multiplier','initialization_code','game_type', 'participant','participant_label', 'round_number', 'id_in_group', 'player_id_index','seconds', 'strategy', 'payoff','multiplied_payoff', 'move', 'remaining_freeze_period', 'if_freeze_next', 'if_freeze_now']
